...
$(function () { 
  ...
  var dtsupervisaoTable = $('.supervisao-list-table'), //id da tabela q esta na div  
    newsupervisaoSidebar = $('.new-supervisao-modal'), //name do modal
    isRtl = $('html').attr('data-textdirection') === 'rtl',
    newsupervisaoForm = $('.add-new-supervisao'); //formula

  // Datatable
  if (dtsupervisaoTable.length) {
    dtsupervisaoTable.DataTable({
      //busca uma rota 
      // ajax: assetPath + 'data/supervisao-list.json', // JSON file to add data
      ajax:{url:"/supervisao/all",dataSrc:""},
      columns: [
      ...
      buttons: [
        ...
        {
          text: feather.icons['plus'].toSvg({ class: 'mr-50 font-small-4 ' }) + 'Nova supervisão',
      ...
      initComplete: function () {
       ...
       .every(function () {
         var column = this;
         var select = $(
           '<select id="supervisaoRole" class="form-control select2 "><option value=""> name </option></select>'
         )
           .appendTo('.supervisao_role')
           .on('change', function () {
    ...
    $('div.head-label').html('<h6 class="mb-0">Listando todas as supervisaos</h6>');
  }
  // Check Validity
  ...
  // Form Validation
  if (newsupervisaoForm.length) {
    newsupervisaoForm.validate({
      errorClass: 'error',
      rules: {
        'name': {
          required: true
  ...
  newsupervisaoForm.on('submit', function (e) {
      var isValid = newsupervisaoForm.valid();
      e.preventDefault();
      if (isValid) {   
        let serealize = newsupervisaoForm.serializeArray();
        console.log(serealize);
        $.ajax({
            type: "POST",
            url: '/supervisao/cadastro',
            data: serealize, 
            success: function(data)
            {
              ...
              if(result[0] == 'Erro'){}else{
                ...
                newsupervisaoSidebar.modal('hide');
              }                
            }
        });        
      }
  ...
  function editarlinha(serealize, data){
    ...
    //  var rowData = dtsupervisaoTable.DataTable().row($('.alteraressatr')).data();  //mostra todos os dados dessa tr;
    dtsupervisaoTable.DataTable().row($('.alteraressatr')).data({
  ...
  function addnovalinha(serealize, data){
    var t = dtsupervisaoTable.DataTable();    
    var rowNode = t
    .row.add( {
      "id":     data,
      "name":   serealize[2]['value'],
 ...
 $(".ediadi").text('Editar');
    ...
    row_edit = dtsupervisaoTable.DataTable().row( $(this).parents('tr')).node(); 
  });
  $(document).on('click','#deletar_td', function(){ 
    var t = dtsupervisaoTable.DataTable();
    var row = dtsupervisaoTable.DataTable().row( $(this).parents('tr') ).node(); 
    var id = $(this).data('id');
    //mensagem de confirmar 
    Swal.fire({
      title: 'Remover a Supervisão',
      text: $(this).data('name') + '?',
      icon: 'warning',
      ...
      if (result.value) {
        $.get('/supervisao/delete/'+id, function(retorno){      
          if(retorno == 'Erro'){
              //mensagem
