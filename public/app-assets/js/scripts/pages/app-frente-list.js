...
$(function () { 
  ...
  var dtfrenteTable = $('.frente-list-table'), //id da tabela q esta na div  
    newfrenteSidebar = $('.new-frente-modal'), //name do modal
    isRtl = $('html').attr('data-textdirection') === 'rtl',
    newfrenteForm = $('.add-new-frente'); //formula

  // Datatable
  if (dtfrenteTable.length) {
    dtfrenteTable.DataTable({
      //busca uma rota 
      // ajax: assetPath + 'data/frente-list.json', // JSON file to add data
      ajax:{url:"/frente/all",dataSrc:""},
      columns: [
      ...
      buttons: [
        ...
        {
          text: feather.icons['plus'].toSvg({ class: 'mr-50 font-small-4 ' }) + 'Nova Frente',
      ...
      initComplete: function () {
       ...
       .every(function () {
         var column = this;
         var select = $(
           '<select id="frenteRole" class="form-control select2 "><option value=""> name </option></select>'
         )
           .appendTo('.frente_role')
           .on('change', function () {
    ...
    $('div.head-label').html('<h6 class="mb-0">Listando todas as frentes</h6>');
  }
  // Check Validity
  ...
  // Form Validation
  if (newfrenteForm.length) {
    newfrenteForm.validate({
      errorClass: 'error',
      rules: {
        'name': {
          required: true
  ...
  newfrenteForm.on('submit', function (e) {
      var isValid = newfrenteForm.valid();
      e.preventDefault();
      if (isValid) {   
        let serealize = newfrenteForm.serializeArray();
  ...
  function editarlinha(serealize, data){
    ...
    //  var rowData = dtfrenteTable.DataTable().row($('.alteraressatr')).data();  //mostra todos os dados dessa tr;
    dtfrenteTable.DataTable().row($('.alteraressatr')).data({
 ...
 $(".ediadi").text('Editar');
    ...
    row_edit = dtfrenteTable.DataTable().row( $(this).parents('tr')).node(); 
  });
  $(document).on('click','#deletar_td', function(){ 
    var t = dtfrenteTable.DataTable();
    var row = dtfrenteTable.DataTable().row( $(this).parents('tr') ).node(); 
    var id = $(this).data('id');
    //mensagem de confirmar 
    Swal.fire({
      title: 'Remover a Frente',
      text: $(this).data('name') + '?',
      icon: 'warning',
      ...
      if (result.value) {
        $.get('/frente/delete/'+id, function(retorno){      
          if(retorno == 'Erro'){
              //mensagem
