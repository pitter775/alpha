...
$(function () { 
  ...
  var dtregimeTable = $('.regime-list-table'), //id da tabela q esta na div  
    newregimeSidebar = $('.new-regime-modal'), //name do modal
    isRtl = $('html').attr('data-textdirection') === 'rtl',
    newregimeForm = $('.add-new-regime'); //formula

  // Datatable
  if (dtregimeTable.length) {
    dtregimeTable.DataTable({
      //busca uma rota 
      // ajax: assetPath + 'data/regime-list.json', // JSON file to add data
      ajax:{url:"/regime/all",dataSrc:""},
      columns: [
      ...
      buttons: [
        ...
        {
          text: feather.icons['plus'].toSvg({ class: 'mr-50 font-small-4 ' }) + 'Novo regime',
      ...
      initComplete: function () {
       ...
       .every(function () {
         var column = this;
         var select = $(
           '<select id="regimeRole" class="form-control select2 "><option value=""> name </option></select>'
         )
           .appendTo('.regime_role')
           .on('change', function () {
    ...
    $('div.head-label').html('<h6 class="mb-0">Listando todas as regimes</h6>');
  }
  // Check Validity
  ...
  // Form Validation
  if (newregimeForm.length) {
    newregimeForm.validate({
      errorClass: 'error',
      rules: {
        'name': {
          required: true
  ...
  newregimeForm.on('submit', function (e) {
      var isValid = newregimeForm.valid();
      e.preventDefault();
      if (isValid) {   
        let serealize = newregimeForm.serializeArray();
  ...
  function editarlinha(serealize, data){
    ...
    //  var rowData = dtregimeTable.DataTable().row($('.alteraressatr')).data();  //mostra todos os dados dessa tr;
    dtregimeTable.DataTable().row($('.alteraressatr')).data({
 ...
 $(".ediadi").text('Editar');
    ...
    row_edit = dtregimeTable.DataTable().row( $(this).parents('tr')).node(); 
  });
  $(document).on('click','#deletar_td', function(){ 
    var t = dtregimeTable.DataTable();
    var row = dtregimeTable.DataTable().row( $(this).parents('tr') ).node(); 
    var id = $(this).data('id');
    //mensagem de confirmar 
    Swal.fire({
      title: 'Remover o Regime',
      text: $(this).data('name') + '?',
      icon: 'warning',
      ...
      if (result.value) {
        $.get('/regime/delete/'+id, function(retorno){      
          if(retorno == 'Erro'){
              //mensagem
