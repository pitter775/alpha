...
$(function () {
   ...
   var dtempresaTable = $('.empresa-list-table'), //id da tabela q esta na div  
      newempresaSidebar = $('.new-empresa-modal'), //name do modal
      isRtl = $('html').attr('data-textdirection') === 'rtl',
      newempresaForm = $('.add-new-empresa'); //formula

   // Datatable
   if (dtempresaTable.length) {
      dtempresaTable.DataTable({
         //busca uma rota 
         // ajax: assetPath + 'data/empresa-list.json', // JSON file to add data
         ajax: { url: "/empresa/all", dataSrc: "" },
         columns: [
         ...
         buttons: [
            ...
            {
               text: feather.icons['plus'].toSvg({ class: 'mr-50 font-small-4 ' }) + 'Nova empresa',
         ...
         initComplete: function () {
            ...
            this.api()
               ...
               .every(function () {
                  var column = this;
                  var select = $(
                     '<select id="empresaRole" class="form-control select2 "><option value=""> name </option></select>'
                  )
                     .appendTo('.empresa_role')
                     .on('change', function () {
      ...
      $('div.head-label').html('<h6 class="mb-0">Listando todas as empresas</h6>');
   }
   // Check Validity
   ...
   // Form Validation
   if (newempresaForm.length) {
      newempresaForm.validate({
         errorClass: 'error',
         rules: {
            'name': {
               required: true
      ...
      newempresaForm.on('submit', function (e) {
         var isValid = newempresaForm.valid();
         e.preventDefault();
         if (isValid) {
            let serealize = newempresaForm.serializeArray();
   ...
   function editarlinha(serealize, data) {
      ...
      //  var rowData = dtempresaTable.DataTable().row($('.alteraressatr')).data();  //mostra todos os dados dessa tr;
      dtempresaTable.DataTable().row($('.alteraressatr')).data({
   ...
   $(document).on('click', '#editar_td', function () {
      ...
      row_edit = dtempresaTable.DataTable().row($(this).parents('tr')).node();
   });
   $(document).on('click', '#deletar_td', function () {
      ...
      Swal.fire({
         ...
         if (result.value) {
            $.get('/empresa/delete/' + id, function (retorno) {
               if (retorno == 'Erro') {
                  //mensagem
