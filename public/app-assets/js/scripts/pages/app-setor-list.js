...
$(function () { 
  ...
  var dtsetorTable = $('.setor-list-table'), //id da tabela q esta na div  
    newsetorSidebar = $('.new-setor-modal'), //name do modal
    isRtl = $('html').attr('data-textdirection') === 'rtl',
    newsetorForm = $('.add-new-setor'); //formula

  // Datatable
  if (dtsetorTable.length) {
    dtsetorTable.DataTable({
      //busca uma rota 
      // ajax: assetPath + 'data/setor-list.json', // JSON file to add data
      ajax:{url:"/setor/all",dataSrc:""},
      columns: [
      ...
      buttons: [
        ...
        {
          text: feather.icons['plus'].toSvg({ class: 'mr-50 font-small-4 ' }) + 'Novo setor',
      ...
      initComplete: function () {
       ...
       .every(function () {
         var column = this;
         var select = $(
           '<select id="setorRole" class="form-control select2 "><option value=""> name </option></select>'
         )
           .appendTo('.setor_role')
           .on('change', function () {
    ...
    $('div.head-label').html('<h6 class="mb-0">Listando todas as setors</h6>');
  }
  // Check Validity
  ...
  // Form Validation
  if (newsetorForm.length) {
    newsetorForm.validate({
      errorClass: 'error',
      rules: {
        'name': {
          required: true
  ...
  newsetorForm.on('submit', function (e) {
      var isValid = newsetorForm.valid();
      e.preventDefault();
      if (isValid) {   
        let serealize = newsetorForm.serializeArray();
  ...
  function editarlinha(serealize, data){
    ...
    //  var rowData = dtsetorTable.DataTable().row($('.alteraressatr')).data();  //mostra todos os dados dessa tr;
    dtsetorTable.DataTable().row($('.alteraressatr')).data({
 ...
 $(".ediadi").text('Editar');
    ...
    row_edit = dtsetorTable.DataTable().row( $(this).parents('tr')).node(); 
  });
  $(document).on('click','#deletar_td', function(){ 
    var t = dtsetorTable.DataTable();
    var row = dtsetorTable.DataTable().row( $(this).parents('tr') ).node(); 
    var id = $(this).data('id');
    //mensagem de confirmar 
    Swal.fire({
      title: 'Remover o Setor',
      text: $(this).data('name') + '?',
      icon: 'warning',
      ...
      if (result.value) {
        $.get('/setor/delete/'+id, function(retorno){      
          if(retorno == 'Erro'){
              //mensagem
