...
$(function () { 
  ...
  var dttarifaTable = $('.tarifa-list-table'), //id da tabela q esta na div  
    newtarifaSidebar = $('.new-tarifa-modal'), //name do modal
    isRtl = $('html').attr('data-textdirection') === 'rtl',
    newtarifaForm = $('.add-new-tarifa'); //formula

  // Datatable
  if (dttarifaTable.length) {
    dttarifaTable.DataTable({
      //busca uma rota 
      // ajax: assetPath + 'data/tarifa-list.json', // JSON file to add data
      ajax:{url:"/tarifa/all",dataSrc:""},
      columns: [
      ...
      buttons: [
        ...
        {
          text: feather.icons['plus'].toSvg({ class: 'mr-50 font-small-4 ' }) + 'Nova tarifa',
      ...
      initComplete: function () {
       ...
       .every(function () {
         var column = this;
         var select = $(
           '<select id="tarifaRole" class="form-control select2 "><option value=""> name </option></select>'
         )
           .appendTo('.tarifa_role')
           .on('change', function () {
    ...
    $('div.head-label').html('<h6 class="mb-0">Listando todas as tarifas</h6>');
  }
  // Check Validity
  ...
  // Form Validation
  if (newtarifaForm.length) {
    newtarifaForm.validate({
      errorClass: 'error',
      rules: {
        'name': {
          required: true
  ...
  newtarifaForm.on('submit', function (e) {
      var isValid = newtarifaForm.valid();
      e.preventDefault();
      if (isValid) {   
        let serealize = newtarifaForm.serializeArray();
  ...
  function editarlinha(serealize, data){
    ...
    //  var rowData = dttarifaTable.DataTable().row($('.alteraressatr')).data();  //mostra todos os dados dessa tr;
    dttarifaTable.DataTable().row($('.alteraressatr')).data({
 ...
 $(".ediadi").text('Editar');
    ...
    row_edit = dttarifaTable.DataTable().row( $(this).parents('tr')).node(); 
  });
  $(document).on('click','#deletar_td', function(){ 
    var t = dttarifaTable.DataTable();
    var row = dttarifaTable.DataTable().row( $(this).parents('tr') ).node(); 
    var id = $(this).data('id');
    //mensagem de confirmar 
    Swal.fire({
      title: 'Remover a Tarifa',
      text: $(this).data('name') + '?',
      icon: 'warning',
      ...
      if (result.value) {
        $.get('/tarifa/delete/'+id, function(retorno){      
          if(retorno == 'Erro'){
              //mensagem
